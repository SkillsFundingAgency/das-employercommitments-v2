...
public class ApprovalsApiClientTests
{
    ...
    [Test, AutoData]
    public async Task When_Calling_GetSelectProviderDetails_Then_The_Data_Is_Returned(long id, GetSelectProviderDetailsResponse response)
    {
        var outerApiClient = new Mock<IOuterApiClient>();
        var expectedUrl = $"selectprovider?accountLegalEntityId={id}";
        outerApiClient.Setup(x => x.Get<GetSelectProviderDetailsResponse>(expectedUrl)).ReturnsAsync(response);
        var approvalsApiClient = new ApprovalsApiClient(outerApiClient.Object);

        var actual = await approvalsApiClient.GetSelectProviderDetails(id);
