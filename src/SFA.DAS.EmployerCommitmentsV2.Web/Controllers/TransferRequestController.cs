    ...
    public class TransferRequestController : Controller
    {
        ...
        public async Task<IActionResult> TransferDetailsForSender(TransferRequestForSenderViewModel viewModel)
        {
            ...
            try
            {
                ...
                return RedirectToAction(nameof(TransferRequestController.TransferConfirmation), nameof(TransferRequestController).ControllerName(), new { viewModel.AccountHashedId, viewModel.TransferRequestHashedId, request.TransferApprovalStatus, viewModel.TransferReceiverName });
            }
            catch (Exception ex)
            {
